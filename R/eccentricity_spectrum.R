# this actually provides the eccentricity spectrum of
...
# errors are thus of the order of 5/128A^4, which perhaps surprisingly is still of the order of 0.015 / 0.020 in eccentricity. 
...
# in fact using higher order terms helps a bit for capturing the 'low eccentricity' bit, but it actually makes things 
...
eccentricity_spectrum <- function(G) {
  ...
  bklm <- 0.75 * as.numeric(apply(IJK, 1, function(V) {
    bk[V[1]] * bk[V[2]] * bk[V[3]]
  }))
  gklm1 <- as.numeric(apply(IJK, 1, function(V) {
    gk[V[1]] + gk[V[2]] + gk[V[3]]
  }))
  gklm2 <- as.numeric(apply(IJK, 1, function(V) {
    gk[V[1]] + gk[V[2]] - gk[V[3]]
  }))
  gklm3 <- as.numeric(apply(IJK, 1, function(V) {
    gk[V[1]] - gk[V[2]] + gk[V[3]]
  }))
  gklm4 <- as.numeric(apply(IJK, 1, function(V) {
    gk[V[1]] - gk[V[2]] - gk[V[3]]
  }))
  pklm1 <- as.numeric(apply(IJK, 1, function(V) {
    pk[V[1]] + pk[V[2]] + pk[V[3]]
  }))
  pklm2 <- as.numeric(apply(IJK, 1, function(V) {
    pk[V[1]] + pk[V[2]] - pk[V[3]]
  }))
  pklm3 <- as.numeric(apply(IJK, 1, function(V) {
    pk[V[1]] - pk[V[2]] + pk[V[3]]
  }))
  pklm4 <- as.numeric(apply(IJK, 1, function(V) {
    pk[V[1]] - pk[V[2]] - pk[V[3]]
  }))

  group[[11]] <- data.frame(
    A = c(bklm, bklm, bklm, bklm),
