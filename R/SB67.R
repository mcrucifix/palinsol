...
SB67_Internal <- function(ell, P0, epsilonbar, zeta = 1.600753 * pi / 180,
  ...
  if (!is.null(PlanetarySolution$i2o)) {
    ...
    TERM3$sigma <- outer(outer(I2OM$sigma, I2OM$sigma[-n],
                               function(si, sj) sj - si), I2OM$sigma, function(sj, sk) sj + sk)

    TERM3$delta <- outer(outer(I2OM$delta, I2OM$delta[-n],
                               function(si, sj) sj - si), I2OM$delta, function(sj, sk) sj + sk)
    ...
    stop("missing either io or i2o in planetary solution")
  }
  ...
  ctemp <- outer(EPI$beta, EPI$beta, function(bi, bk) {
    cos(bi - bk)
  })
  dij <- outer(seq(ng), seq(ng), function(i, k) {
    i < k
  })
  ...
  for (iter in seq(itermax)) {
    ...
    if (ber90_reproduce) {
      ...
      Constraint <- (1 - sum(IOM$N^2 * (1.5 + 0.75 * ci^2 - 2.5 * ci -
                                        0.5 * (ci * (ci - 1)) * tane^2))
    - 3 * P0 / ell * sum(temp * ctemp * dij, na.rm = TRUE))
  ...
  D2fik <- psi * outer(fi, fi, function(fi, fk) {
    1 / (fi + fk)
  }) * (
    0.5 * outer(ci, ci, function(ci, ck) {
      ci^2 + ck^2 + ci + ck - ci * ck - 2
    })
    - C2fik * tane
      - 0.5 * outer(ci, ci, function(ci, ck) {
        ci * (ci - 1) + ck * (ck - 1)
  ...
  D2fikprime <- psi * outer(fi, fi, function(fi, fk) {
    1 / (fi - fk)
  }) * (
    0.5 * outer(ci, ci, function(ci, ck) {
      5 * (ci + ck) - (ci^2 + ck^2) - ci * ck - 6
    })
    - C2fikprime * tane
  ...
  Dfseconde <- 3 * psi * P0 / ell * outer(EPI$g, EPI$g, function(gi, gk) {
    1. / (gi - gk)
  })
  ...
  OBLIQUITY <- list(
    N = c(ETERM1$N, ETERM2$N, ETERM3$N, ETERM4$N),
  ...
  PTERM1 <- list(
    Amp = Dfseconde * outer(EPI$M, EPI$M),
  ...
  PTERM5 <- list(
    Amp = D2ikprime * outer(IOM$N, IOM$N),
    Freq = outer(fi, fi, "-"),
    Phases = outer(deltaprime, deltaprime, "-")
  )
  ...
  CTERM2 <- list(
    Amp = as.numeric(outer(EPI$M, D1 * IOM$N)),
